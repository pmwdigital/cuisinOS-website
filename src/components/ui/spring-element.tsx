...
function SpringElement({
    ...
    const drawSpring = React.useCallback(() => {
        const element = pathRef.current;
        if (!element) return;
        const { x: cx, y: cy } = centerRef.current;
        element.setAttribute(
            "d",
            generateSpringPath(cx, cy, cx + springX.get(), cy + springY.get(), springPathConfig)
        );
    }, [springPathConfig, springX, springY]);
    ...
    React.useEffect(() => {
        ...
        const unsubscribeX = springX.on("change", drawSpring);
        const unsubscribeY = springY.on("change", drawSpring);
        ...
        <>
            ...
            >
                <path
                    ...
                    className={cn("fill-none stroke-chocolate stroke-2", springClassName)}
                />
            </svg>
            <motion.div
                ref={childRef}
