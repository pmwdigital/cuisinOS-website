            ...
            <div className="flex flex-col items-center justify-center max-w-2xl mx-auto">
                <Container>
                    <div className="flex flex-col items-center text-center max-w-2xl mx-auto">
                        <h2 className="text-2xl md:text-4xl lg:text-5xl font-heading font-normal !leading-snug mt-6">
                            İşletmenize uyan <br className="hidden lg:block" /> planı seçin
                        </h2>
                ...
                <Container delay={0.2}>
                    <div className="flex items-center justify-center space-x-4 mt-6">
                        <span className="text-base font-medium">Aylık</span>
                        <button onClick={handleSwitch} className="relative rounded-full focus:outline-none">
                            <div className="w-12 h-6 transition rounded-full shadow-md outline-none bg-grapefruit"></div>
                            <div
                                className={cn(
                        ...
                        <span className="text-base font-medium">Yıllık</span>
                    </div>
                </Container>
            </div>
...
const Plan = ({ plan, billPlan }: { plan: PLAN, billPlan: Plan }) => {
    return (
        <div className={cn(
            ...
            plan.title === "Pro" && "border-grapefruit"
        )}>
            {plan.title === "Pro" && (
                <div className="absolute top-1/2 inset-x-0 mx-auto h-12 -rotate-45 w-full bg-grapefruit-dark rounded-2xl lg:rounded-3xl blur-[8rem] -z-10"></div>
            )}
            ...
            <div className="flex flex-col items-start w-full px-4 py-2 md:px-8">
                <Button size="lg" variant={plan.title === "Pro" ? "brand" : "white"} className="w-full">
                    {plan.buttonText}
                </Button>
